...
echo "  MAX_PARALLEL_JOBS=4 CUDA_DEVICES='0,1,2,3' ./resnet50_all.sh"
...
# START=0
# END=2048
# INDEX_LIST=$(seq $START $((END - 1)));
# echo $INDEX_LIST

# NUM=200
# # Sample NUM indices from INDEX_LIST
# SAMPLED_INDEX_LIST=$(echo $INDEX_LIST | tr ' ' '\n' | shuf -n $NUM | tr '\n' ' ')
# echo "Sampled indices: $SAMPLED_INDEX_LIST"


# python src/experiments/preprocessing/crop_activation_regions.py \
#     --config_file /project/src/configs/imagenet/resnet50_timm.yaml \
#     --indices_file /project/results/top_activations/top10pct/top_activations_layer4_2_output_indices.npy \
#     --activation_file /project/results/activations/imagenet/imagenet/vit_b_16_timm/activations_layer4_2_output_raw.safetensors \
#     --output_dir /project/results/cropped_regions/resnet50_timm \
#     --neuron_indices ${SAMPLED_INDEX_LIST} \
#     --top_k_samples 100 \
...
# echo "Step 0: Extracting raw activations for each layer in parallel..."

# extract_activations_per_layer() {
#     local layers=("$@")
#     local job_index=0
...
#         local cuda_device
#         cuda_device=$(get_cuda_device $job_index)

#         echo "Extracting activations for layer: $layer on CUDA device $cuda_device"
...
# extract_activations_per_layer "${LAYER_LIST[@]}"
...
process_layer() {
    ...
    LAYER="${LAYER_LIST[$i]}"
    SAFE_LAYER="${LAYER//./_}" # For file names (e.g., features.11 -> features_11)
    
    PREV_NUM=$((i - 1))
    PREV_LAYER="${LAYER_LIST[$PREV_NUM]}"
    ...
    CUDA_VISIBLE_DEVICES=$cuda_device python /project/src/experiments/preprocessing/compute_top_activations.py \
        ...
        --aggregation top_mean \
        --top_percentile 10.0 \
        --output_dir "/project/results/top_activations/imagenet/$MODEL_NAME/top10pct_1000/" \
        --save_values
    ...
    PREV_LAYER="layer4.1"
    SAFE_LAYER="layer4_2"
    CUDA_VISIBLE_DEVICES=1 python /project/src/experiments/disentangling/attribution.py \
        --model_name $MODEL_NAME \
        --tgt_layer_name "$LAYER" \
        --src_layer_name "$PREV_LAYER" \
        --neuron_indices 50 100 200 300 400 500 600 700 800 \
        --top_index_file "/project/results/top_activations/imagenet/$MODEL_NAME/top10pct_1000/top_activations_${SAFE_LAYER}_output_indices.npy" \
        --output_dir "/project/results/attributions/main_figure"

    MODEL_NAME="resnet50_timm"
    LAYER="layer4.2"
    SAFE_LAYER="layer4_2"
    CUDA_VISIBLE_DEVICES=1 python /project/src/experiments/disentangling/greedy_clustering.py \
        --model_name $MODEL_NAME \
        --tgt_layer_name "$LAYER" \
        --attr_dir "/project/results/attributions/main_figure/$MODEL_NAME/${LAYER}" \
        --top_index_file "/project/results/top_activations/imagenet/$MODEL_NAME/top10pct_1000/top_activations_${SAFE_LAYER}_output_indices.npy" \
        --neuron_indices 50 100 200 300 400 500 600 700 800 \
        --num_samples 1000 \
        --output_dir "/project/results/clustering/main_figure/$MODEL_NAME"
