...
echo "  MAX_PARALLEL_JOBS=4 CUDA_DEVICES='0,1,2,3' ./vit_s_16.sh"

MODEL_NAME="vgg19_timm"
LAYER_LIST=(
    "features.1"   # 64
    "features.3"   # 64
    "features.6"   # 128
...
python /project/src/experiments/preprocessing/extract_activations.py \
    --layers_to_hook features.1 output features.3 output features.6 output features.8 output features.11 output features.13 output features.15 output features.18 output features.20 output features.22 output features.25 output features.27 output features.29 output features.32 output features.34 output features.36 output \
    --batch_size 256 \
    --pool_type raw \
