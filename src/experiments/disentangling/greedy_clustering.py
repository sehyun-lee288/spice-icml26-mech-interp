...
def main():
    ...
    for neuron_idx in tqdm(neuron_indices_to_process):
        ...
        mean = neuron_attribution.mean(dim=0)
        std = neuron_attribution.std(dim=0)
        std = std.clone()
        eps = 1e-6
        std = torch.where(std < eps, torch.tensor(1.0, device=std.device, dtype=std.dtype), std)
        neuron_attribution = (neuron_attribution - mean) / std
        print (f"Neuron attribution is loaded.")
